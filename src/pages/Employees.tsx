...
const empty = { full_name: '', employee_code: '', email: '', password: '', department: '', role: 'staff' as AppRole }
...
export default function Employees() {
  ...
  async function load(){ const {data,error}=await supabase.from('profiles').select('id,full_name,email,employee_code,role,department,active,archived_at').order('created_at',{ascending:false}); if(error)setMessage(error.message); else setRows((data||[]) as Profile[]) }
  ...
  return <>
    ...
    <form className="card form-grid" onSubmit={create}>
      ...
      <div className="wide actions"><span className="muted">Tài khoản mới được xác nhận email tự động.</span><button className="primary" disabled={busy}>Tạo nhân viên</button></div>
    </form>
    ...
    <div className="table-wrap card"><table><thead><tr><th>Mã NV</th><th>Họ tên</th><th>Email</th><th>Bộ phận</th><th>Quyền</th><th>Trạng thái</th><th></th></tr></thead><tbody>
      {visible.map(r=><tr key={r.id}><td>{r.employee_code||'—'}</td><td>{r.full_name||'—'}</td><td>{r.email||'—'}</td><td>{r.department||'—'}</td><td>{r.role}</td><td>{r.archived_at?'Đã lưu trữ':r.active?'Đang hoạt động':'Đã khóa'}</td><td><button className="ghost small" onClick={()=>setEditing(r)}>Quản lý</button></td></tr>)}
    </tbody></table></div>
    {editing&&<div className="modal-backdrop"><div className="modal card"><div className="modal-head"><h2>Quản lý nhân viên</h2><button className="ghost" onClick={()=>setEditing(null)}>Đóng</button></div>
      ...
      <div className="stack-actions">
        <button className="primary" disabled={busy} onClick={()=>act({action:'update',user_id:editing.id,full_name:editing.full_name,employee_code:editing.employee_code,department:editing.department,role:editing.role},'Đã cập nhật nhân viên.')}>Lưu thay đổi</button>
