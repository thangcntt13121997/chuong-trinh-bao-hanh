...
const VALID_SERVICE_TYPES=new Set(['free_warranty','paid_warranty','free_exchange','paid_exchange'])
const VALID_CAUSES=new Set(['technical','user','unknown'])
const VALID_STATUSES=new Set(['received','pending_supplier','sent_supplier','supplier_returned','ready_for_customer','returned_customer','cancelled'])
const SERVICE_ALIASES={warranty:'free_warranty',repair:'paid_warranty',exchange:'free_exchange',inspection:'free_warranty'}
const CAUSE_ALIASES={physical:'unknown',accessory:'unknown',usage:'user',supplier:'unknown',other:'unknown'}
const STATUS_ALIASES={processing:'received',waiting_supplier:'pending_supplier',ready_return:'ready_for_customer',returned:'returned_customer',closed:'returned_customer'}
const normalizeService=v=>{const x=String(v||'free_warranty');const n=SERVICE_ALIASES[x]||x;return VALID_SERVICE_TYPES.has(n)?n:'free_warranty'}
const normalizeCause=v=>{const x=String(v||'unknown');const n=CAUSE_ALIASES[x]||x;return VALID_CAUSES.has(n)?n:'unknown'}
const normalizeStatus=v=>{const x=String(v||'received');const n=STATUS_ALIASES[x]||x;return VALID_STATUSES.has(n)?n:'received'}
  ...
  try{
    ...
    if(action==='dashboard'){
      ...
      const [open,waiting,appointments,overdue,recent,upcoming]=await Promise.all([
        db.from('service_cases').select('*',{count:'exact',head:true}).is('archived_at',null).not('status','in','("returned_customer","cancelled")'),
        db.from('service_cases').select('*',{count:'exact',head:true}).is('archived_at',null).in('status',['pending_supplier','sent_supplier']),
        db.from('service_appointments').select('*',{count:'exact',head:true}).gte('scheduled_at',now).lte('scheduled_at',soon).eq('status','scheduled'),
        db.from('service_cases').select('*',{count:'exact',head:true}).is('archived_at',null).lt('promised_return_at',now).not('status','in','("returned_customer","cancelled")'),
